...
SubGridEvents.Events = {
  ...
  ValidateEstimatedRevenue: function(field, value){
    if(field.key == "estimatedvalue" && Number(value)>100000){
        // show custom alert via alertbox if available, otherwise fallback to window.alert
  ...
  CustomAlert: function(Icon, title, message){
    if (typeof alertbox !== "undefined" && typeof alertbox.render === "function") {
      alertbox.render({
          alertIcon: Icon,
          title: title,
          message: message,
...
// --- Unified Save for non-lookup fields (and safety check for lookups) ---
async function saveEdit(tr, level, record, field, input, td) {
  ...
  // If caller tries to save a lookup via saveEdit, redirect to saveLookupEdit if selectedId exists.
  if (field.type === "lookup") {
    const selectedId = input && input.dataset ? input.dataset.selectedId : null;
    if (!selectedId) {
      // If no selected id, do not proceed — require selecting the dropdown item.
      if (typeof SubGridEvents !== "undefined" && typeof SubGridEvents.Events !== "undefined" && typeof SubGridEvents.Events.CustomAlert === "function") {
        SubGridEvents.Events.CustomAlert('error', 'Select Lookup', 'Please select a record from the dropdown.');
      } else {
  ...
  // calling ValidateGrid (includes estimated revenue validation)
  ...
  try {
    ...
    // Provide the raw error message for easier debugging
...
async function startEditCell(tr, level, record, field, td) {
    ...
    if (field.type === "lookup") {
        ...
        input.dataset.selectedId = ""; // will be filled when user selects
        ...
        input.addEventListener("input", () => {
            // Clear any previously selected id when user types a new search
            ...
            timeout = setTimeout(async () => {
                ...
                currentResults.forEach(r => {
                    ...
                    item.onclick = () => {
                        // when user clicks, set both visible display and the selectedId
                        input.value = r.display;
                        input.dataset.selectedId = r.id;
                        // Save using the proper odata.bind payload
        ...
        input.onkeydown = (ev) => {
            ...
            if (ev.key === "Enter") {
                ...
                if (selectedId) {
                  // user has selected a result -> save
                  saveLookupEdit(level, record, field, selectedId, td);
                } else {
                  // do not allow saving a free-text value for lookups
                  if (typeof SubGridEvents !== "undefined" && typeof SubGridEvents.Events !== "undefined" && typeof SubGridEvents.Events.CustomAlert === "function") {
                      SubGridEvents.Events.CustomAlert('error', 'Select Lookup', 'Please select a record from the dropdown.');
                  } else {
                      alert("Please select a record from the dropdown.");

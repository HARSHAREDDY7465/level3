...
const quoteColumns = [
  ...
  { key: "statecode", label: "Status", editable: true, type: "option" }
];
...
async function fetchLookupOptions(entitySet, searchText, displayField) {
  ...
  return await fetchData(entitySet, `${displayField},${entitySet.slice(0, -1)}id`, filter);
}

// --- Start Editing Cell with Lookup ---
async function startEditCell(tr, level, record, field, td) {
  ...
  if (field.type === "choice") {
    ...
    // Option set handling can be added here (not shown for brevity)
    ...
    input.addEventListener("input", async (e) => {
      dropdown.innerHTML = "";
      const options = await fetchLookupOptions(field.lookup.entitySet, input.value, field.lookup.displayField);
      options.forEach(opt => {
        const item = document.createElement("div");
        item.textContent = opt[field.lookup.displayField];
        item.style.padding = "4px";
        item.style.cursor = "pointer";
        item.addEventListener("click", async () => {
          const updateObj = {};
          updateObj[`${field.key}@odata.bind`] = `/${field.lookup.entitySet}(${opt[`${field.lookup.entitySet.slice(0, -1)}id`]})`;
          await patchData(hierarchyConfig[level].entitySet, record[hierarchyConfig[level].key], updateObj);
          editingCell = null;
          await renderGrid();
        });
    ...
    document.addEventListener("click", (ev) => {
      if (!td.contains(ev.target)) dropdown.innerHTML = "";
    });
    return;
  } else {
    input = document.createElement("input");

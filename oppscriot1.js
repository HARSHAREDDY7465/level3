...
function updateButtons() {
  ...
  function renderQuestions(items) {
    ...
    items.forEach((item) => {
      ...
      if (question.answerdatatype === "Lookup") {
        console.log(`Stored lookup metadata for ${question.questionid}:`, {
          entityName: question.lookupEntityName,
          displayFields: question.lookupDisplayFields
        });
      }//Melky
    ...
    Object.keys(grouped).forEach((subsection) => {
      const questionsHtml = grouped[subsection]
        .map((q) => {
          ...
          return `
  <div class="mb-3 question-block" data-question-block-id="${q.questionid
...
`;
    ...
    textInputs.forEach((input) => {
      ...
      const updateCharCount = () => {
        ...
        // Only validate if field has content (1+ characters)
      ...
      input.addEventListener("blur", updateCharCount);
...
case "Lookup": {
  ...
  `;
      ...
      case "File": {
        const acceptValue = getFileAcceptValue(question);
        return `
    <div>
      <input
        type="file"
